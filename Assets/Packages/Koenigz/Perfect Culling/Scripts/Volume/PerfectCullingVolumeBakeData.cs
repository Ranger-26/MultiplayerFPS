...
namespace Koenigz.PerfectCulling
{
    ...
    public class PerfectCullingVolumeBakeData : PerfectCullingBakeData
    {
        ...
        void SetDataCompressed(int index, ushort[] indices)
        {
	        if (indices.Length <= 0)
	        {
		        //Debug.LogWarning($"Baked cell {index} without any renderers visible. This should be very unlikely.");
            ...
            for (int i = 0; i < deltaValues.Count; ++i)
            {
	            ...
	            m_bitStreamWriter.Write((uint)bitIndex, HeaderBitSize);
	            m_bitStreamWriter.Write((uint)deltaValues[i], (int) BITS[bitIndex]);
            }
            
            m_bitStreamWriter.Flush();

            byte[] finalCompressedData = new byte[m_bitStreamWriter.Length];
            System.Array.Copy(m_bitStreamWriter.Buffer, finalCompressedData, m_bitStreamWriter.Length);
...
#pragma warning restore 1998
        ...
        {
	        m_bitStreamReader.Reset(data[index].compressed);
	        ...
	        for (int i = 0; i < len; ++i)
	        {
		        uint bitIndex = m_bitStreamReader.Read(HeaderBitSize);

		        uint numberOfBits = BITS[bitIndex];
		        uint value = m_bitStreamReader.Read((int) numberOfBits);
        ...
        private static BitStreamWriter m_bitStreamWriter = new BitStreamWriter();
